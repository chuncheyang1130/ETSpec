"""SubSpec SVD Recipe: HQQ 4-bit quant (attn + MLP) + SVD without offloading."""
...
class Recipe(BaseRecipe):
    ...
    def generate_configurations(
        ...
        mlp_svd_quant_config = BaseQuantizeConfig(nbits=4, group_size=64, axis=1)
        ...
        for i in range(quant_start, quant_end):
            ...
            quant_config[f"model.layers.{i}.mlp.u_gate_up_proj"] = mlp_svd_quant_config
            quant_config[f"model.layers.{i}.mlp.vh_gate_up_proj"] = mlp_svd_quant_config
            quant_config[f"model.layers.{i}.mlp.down_proj"] = mlp_quant_config
