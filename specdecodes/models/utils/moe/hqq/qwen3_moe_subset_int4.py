...
It keeps the full router for global-id bookkeeping and an `[M, ...]` store indexed
by retained local slot. Routing and forward behavior are inherited from
`_StackedSubsetBase`; this class supplies INT4 storage and kernels.
"""
...
class Qwen3MoeSubsetInt4Block(_StackedSubsetBase):
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__(
            ...
            top_k=top_k, norm_topk_prob=norm_topk_prob, kept=kept,
    ...
    @classmethod
    @torch.no_grad()
    def from_huggingface(cls, hf_block, *, kept_ids, group_size: int = 128,
        ...
        return cls._build(g, u, d, router_module.weight.detach(), kept_ids, int(hf_block.top_k),
                          bool(hf_block.norm_topk_prob), group_size, compute_dtype)
    ...
    def _build(cls, g, u, d, router_w, kept_ids, top_k, norm_topk_prob,
        ...
        block = cls(
            ...
            norm_topk_prob=norm_topk_prob, kept=M,
